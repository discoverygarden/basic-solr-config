        ...
        <xsl:for-each select="$content//cwrc/workflow/activity/@*">
            <xsl:choose>
                <xsl:when test="../../@workflowID=$newest">
                    <field>
                        <xsl:attribute name="name">
                            <xsl:value-of select="concat($prefix,'activity_',local-name(), $current, $suffix)"/>
                        </xsl:attribute>
                        <xsl:value-of select="."/>
                    </field>
                </xsl:when>
                <xsl:otherwise>
                    <field>
                        <xsl:attribute name="name">
                            <xsl:value-of select="concat($prefix,'activity_',local-name(), $superceded, $suffix)"/>
                        </xsl:attribute>
                        <xsl:value-of select="."/>
                    </field>
                </xsl:otherwise>
        ...
        <xsl:for-each select="$content//cwrc/workflow/assigned/@*">
            ...
            <xsl:choose>
                <xsl:when test="../../@workflowID=$newest">
                    <field>
                        <xsl:attribute name="name">
                            <xsl:value-of select="concat($prefix,'assigned_',local-name(), $current, $suffix)"/>
                        </xsl:attribute>
                        <xsl:value-of select="."/>
                    </field>
                </xsl:when>
                <xsl:otherwise>
                    <field>
                        <xsl:attribute name="name">
                            <xsl:value-of select="concat($prefix,'assigned_',local-name(), $superceded, $suffix)"/>
                        </xsl:attribute>
                        <xsl:value-of select="."/>
                    </field>
                </xsl:otherwise>
        ...
        <xsl:for-each select="$content//cwrc/workflow/@*">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <field>
                        <xsl:attribute name="name">
                            <xsl:value-of select="concat($prefix,local-name(), $superceded, $suffix)"/>
                        </xsl:attribute>
                        <xsl:value-of select="."/>
                    </field>
                </xsl:otherwise>
