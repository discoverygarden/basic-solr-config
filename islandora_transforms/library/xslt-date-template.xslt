...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  xmlns:java="http://xml.apache.org/xalan/java" xmlns:xlink="http://www.w3.org/1999/xlink">
  ...
  <xsl:template name="get_ISO8601_date">
    ...
    <xsl:variable name="pattern1">
      ...
      <xsl:variable name="min_el">(\:[0-9]{2}(<xsl:value-of select="$frac"/>|<xsl:value-of
          select="$sec_el"/>?))</xsl:variable>
      <xsl:variable name="time_el">([0-9]{2}(<xsl:value-of select="$frac"/>|<xsl:value-of
          select="$min_el"/>))</xsl:variable>
      <xsl:variable name="time_offset">(Z|[+-]<xsl:value-of select="$time_el"/>)</xsl:variable>
      <xsl:variable name="time_pattern">(T<xsl:value-of select="$time_el"/><xsl:value-of
          select="$time_offset"/>?)</xsl:variable>
      ...
      <xsl:variable name="date_opt_pattern">(<xsl:value-of select="$date_el"/><xsl:value-of
          select="$time_pattern"/>?)</xsl:variable>
      ...
      <xsl:choose>
        <xsl:when test="java:matches($date, $pattern2)">
          <xsl:variable name="dp"
            select="java:org.joda.time.format.DateTimeFormat.forPattern('M/d/y')"/>
          <xsl:value-of select="java:parseDateTime($dp, $date)"/>
        </xsl:when>
        <xsl:when test="java:matches($date, $pattern3)">
          <xsl:variable name="dp"
            select="java:org.joda.time.format.DateTimeFormat.forPattern('M/d/y H:m')"/>
          <xsl:value-of select="java:parseDateTime($dp, $date)"/>
        </xsl:when>
        <xsl:when test="java:matches($date, $pattern4)">
          <xsl:variable name="dp" select="java:org.joda.time.format.DateTimeFormat.forPattern('yyyy')"/>
    ...
    <xsl:choose>
      <xsl:when test="java:matches($asdf, $pattern1)">
        <xsl:variable name="dp"
          select="java:org.joda.time.format.ISODateTimeFormat.dateTimeParser()"/>
